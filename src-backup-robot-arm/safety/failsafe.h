...
// input. None of the other modules (drive, arm_base, the four joint
// modules) know or care whether their input is trustworthy — they just do
// what they're told each time their update() function is called. This
// module decides, once per loop() iteration, which case applies.
// -----------------------------------------------------------------------------

// How stale controller data (input.lastUpdateMs) is allowed to get before
// failsafe triggers. Matches the value from an earlier working prototype.
